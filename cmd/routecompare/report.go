...
func renderTextRoutes(w io.Writer, title string, routes []routecompare.Route) {
	...
	tw := tabwriter.NewWriter(w, 0, 4, 2, ' ', 0)
	fmt.Fprintln(tw, "DESTINATION\tNEXT HOPS\tTYPE\tPROTOCOL\tPREFERENCE\tTABLE\tATTRIBUTES")
	for _, route := range routes {
		fmt.Fprintf(tw, "%s\t%s\t%s\t%s\t%s\t%s\t%s\n", route.Destination, formatNextHops(route.NextHops), route.NextHopType, route.Protocol, route.Preference, route.Table, formatAttributes(route))
	}
	_ = tw.Flush()
}
...
func renderTextModified(w io.Writer, changes []routecompare.RouteChange) {
	...
	for _, change := range changes {
		...
		fmt.Fprintf(w, "  before: protocol=%s preference=%s type=%s next-hops=%s attributes=%s\n", change.Before.Protocol, change.Before.Preference, change.Before.NextHopType, formatNextHops(change.Before.NextHops), formatAttributes(change.Before))
		fmt.Fprintf(w, "  after:  protocol=%s preference=%s type=%s next-hops=%s attributes=%s\n", change.After.Protocol, change.After.Preference, change.After.NextHopType, formatNextHops(change.After.NextHops), formatAttributes(change.After))
	}
}
...
func renderMarkdown(w io.Writer, result report) error {
	...
	if result.Metadata.Comparison != "" {
		fmt.Fprintf(w, "**Comparison:** %s  \n", markdownEscape(result.Metadata.Comparison))
	}
	if result.Metadata.ChangeID != "" {
	...
	for _, change := range result.Modified {
		...
		fmt.Fprintln(w, "\n| | Protocol | Preference | Next-hop type | Next hops | Attributes |")
		fmt.Fprintln(w, "| --- | --- | --- | --- | --- | --- |")
		fmt.Fprintf(w, "| Before | %s | %s | %s | %s | %s |\n", markdownEscape(change.Before.Protocol), markdownEscape(change.Before.Preference), markdownEscape(change.Before.NextHopType), markdownEscape(formatNextHops(change.Before.NextHops)), markdownEscape(formatAttributes(change.Before)))
		fmt.Fprintf(w, "| After | %s | %s | %s | %s | %s |\n", markdownEscape(change.After.Protocol), markdownEscape(change.After.Preference), markdownEscape(change.After.NextHopType), markdownEscape(formatNextHops(change.After.NextHops)), markdownEscape(formatAttributes(change.After)))
	}
	fmt.Fprintln(w, "\n---")
...
func renderMarkdownRoutes(w io.Writer, title string, routes []routecompare.Route) {
	...
	fmt.Fprintln(w, "\n| Destination | Table | Protocol | Preference | Next-hop type | Next hops | Attributes |")
	fmt.Fprintln(w, "| --- | --- | --- | ---: | --- | --- | --- |")
	for _, route := range routes {
		fmt.Fprintf(w, "| %s | %s | %s | %s | %s | %s | %s |\n", markdownEscape(route.Destination), markdownEscape(route.Table), markdownEscape(route.Protocol), markdownEscape(route.Preference), markdownEscape(route.NextHopType), markdownEscape(formatNextHops(route.NextHops)), markdownEscape(formatAttributes(route)))
	}
}
...
{{if or .Metadata.Comparison .Metadata.Device .Metadata.ChangeID}}<div class="meta">{{if .Metadata.Comparison}}<div><div class="label">Comparison</div><strong>{{.Metadata.Comparison}}</strong></div>{{end}}{{if .Metadata.Device}}<div><div class="label">Device</div><strong>{{.Metadata.Device}}</strong></div>{{end}}{{if .Metadata.ChangeID}}<div><div class="label">Change ID</div><strong>{{.Metadata.ChangeID}}</strong></div>{{end}}</div>{{end}}
...
<h2>Added routes ({{len .Added}})</h2><div class="section"><table><thead><tr><th>Destination</th><th>Table</th><th>Protocol</th><th>Preference</th><th>Type</th><th>Next hops</th><th>Attributes</th></tr></thead><tbody>{{range .Added}}<tr><td><code>{{.Destination}}</code></td><td>{{.Table}}</td><td>{{.Protocol}}</td><td>{{.Preference}}</td><td>{{.NextHopType}}</td><td>{{hops .NextHops}}</td><td>{{attrs .}}</td></tr>{{else}}<tr><td colspan="7" class="muted">No displayed routes</td></tr>{{end}}</tbody></table></div>
<h2>Removed routes ({{len .Removed}})</h2><div class="section"><table><thead><tr><th>Destination</th><th>Table</th><th>Protocol</th><th>Preference</th><th>Type</th><th>Next hops</th><th>Attributes</th></tr></thead><tbody>{{range .Removed}}<tr><td><code>{{.Destination}}</code></td><td>{{.Table}}</td><td>{{.Protocol}}</td><td>{{.Preference}}</td><td>{{.NextHopType}}</td><td>{{hops .NextHops}}</td><td>{{attrs .}}</td></tr>{{else}}<tr><td colspan="7" class="muted">No displayed routes</td></tr>{{end}}</tbody></table></div>
<h2>Modified routes ({{len .Modified}})</h2>{{range .Modified}}<div class="section change"><strong><code>{{.Before.Destination}}</code></strong> · {{.Before.Table}}<div class="muted">Changed: {{join .ChangedFields}}</div><table><thead><tr><th></th><th>Protocol</th><th>Preference</th><th>Type</th><th>Next hops</th><th>Attributes</th></tr></thead><tbody><tr class="before"><td>Before</td><td>{{.Before.Protocol}}</td><td>{{.Before.Preference}}</td><td>{{.Before.NextHopType}}</td><td>{{hops .Before.NextHops}}</td><td>{{attrs .Before}}</td></tr><tr class="after"><td>After</td><td>{{.After.Protocol}}</td><td>{{.After.Preference}}</td><td>{{.After.NextHopType}}</td><td>{{hops .After.NextHops}}</td><td>{{attrs .After}}</td></tr></tbody></table></div>{{else}}<div class="section muted">No displayed routes</div>{{end}}
</main></body></html>`))
...
func formatNextHops(nextHops []routecompare.NextHop) string {
	...
	for i, hop := range nextHops {
		parts := []string{hop.To, hop.Via, hop.LocalInterface}
		if hop.Label != "" {
			parts = append(parts, "label="+hop.Label)
		}
		if hop.Selected {
			parts = append(parts, "selected")
		}
		hops[i] = strings.Trim(strings.Join(parts, " "), " ")
	}
	return strings.Join(hops, ", ")
}

func formatAttributes(route routecompare.Route) string {
	var attributes []string
	if route.Active {
		attributes = append(attributes, "active")
	}
	if route.Hidden {
		attributes = append(attributes, "hidden")
	}
	for _, attribute := range []struct{ name, value string }{
		{"metric", route.Metric}, {"metric2", route.Metric2},
		{"local-pref", route.LocalPreference}, {"as-path", route.ASPath},
		{"communities", strings.Join(route.Communities, ",")}, {"tag", route.Tag},
	} {
		if attribute.value != "" {
			attributes = append(attributes, attribute.name+"="+attribute.value)
		}
	}
	if len(attributes) == 0 {
		return "—"
